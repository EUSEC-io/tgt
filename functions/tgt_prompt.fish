# Render a [scenario:target[:port]] indicator for use in fish_prompt
# or fish_right_prompt. Empty output when no scenario is active.
#
# Color reflects damage potential:
#   red     credentials loaded ($TGT_PASSWORD set)
#   yellow  host or port set, no creds
...
#       tgt_prompt
#   end
function tgt_prompt --description 'Render a [scenario:target[:port]] indicator'
    ...
    if set -q TGT_PASSWORD
        set_color red
    else if set -q TGT; or set -q TGT_PORT
        set_color yellow
    else
        set_color normal
    end
