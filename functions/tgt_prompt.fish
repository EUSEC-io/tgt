# Render a [scenario:target[:port][@dc][+cred]] indicator for use in
# fish_prompt or fish_right_prompt. Empty output when no scenario
# is active.
#
# Color reflects damage potential:
#   red     credentials loaded ($TGT_CRED_NAME set)
...
#       tgt_prompt
#   end
function tgt_prompt --description 'Render a [scenario:target[:port][@dc][+cred]] indicator'
    ...
    set -q TGT_CRED_NAME; and set label "$label+$TGT_CRED_NAME"
