...
# Preserves case. Replaces any chars outside [A-Z a-z 0-9 _ -]
# with `-`, collapses runs of `-`, trims leading/trailing `-`.
# Returns "unnamed" if nothing usable is left (e.g., an empty /
# dots-only input).
function _tgt_scenario_sanitize_name --argument-names raw
    set -l name (string replace -ra '[^A-Za-z0-9_-]' '-' -- $raw)
