...
| Cross-shell live updates | SSE on `/api/events` (Phase 2c) + 60 s belt-and-braces poll | Shipped | Raw `EventSource` against `http.server` | Stdlib `text/event-stream`. The htmx pairing never happened; raw `EventSource` is one `<script>`-free function. A slow fallback poll stays because hash dedup makes it nearly free and catches the rare silent-disconnect case. Watcher polls `$TGT_HOME` mtimes + re-probes the active fish universal var every ~1.5 s; if the fish probe gets too costly, gate it behind a `~/.config/fish/fish_variables` mtime check. |
